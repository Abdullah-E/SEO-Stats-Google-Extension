...
console.log(jsonObj)
    //get button with id with id="btn"
    const btn = document.getElementById("btn");
    console.log(btn)
    btn.addEventListener("click", () => {
        //get ketwords:
    let xlsx_fileaname = "data.xlsx"
    let wb = XLSX.utils.book_new()

    let results = jsonObj["tasks"][0]["result"]
    console.log(results.length)
    let rows = []
    const headings = ["keyword", "competition", "competition_index", "search_volume"]
    rows.push(headings)
    for(let i = 0; i < results.length; i++) {
        let row = []
        for(let j = 0; j < headings.length; j++) {
            row.push(results[i][headings[j]])
        }
        rows.push(row)
    }
    ...
    let monthly_rows = []
    let heading_row = []
    heading_row.push("keyword")
    for(let i = 0; i < results[0].monthly_searches.length; i++) {
        heading_row.push(results[0].monthly_searches[i].year + "-" + results[0].monthly_searches[i].month)
    }

    monthly_rows.push(heading_row)
    for(let i = 0; i < results.length; i++) {
        let row = []
        row.push(results[i].keyword)
        for(let j = 0; j < results[i].monthly_searches.length; j++) {
            row.push(results[i].monthly_searches[j].search_volume)
        }
        monthly_rows.push(row)
    }
    console.log(monthly_rows)
    let ws2 = XLSX.utils.aoa_to_sheet(monthly_rows)
    XLSX.utils.book_append_sheet(wb, ws2, "Monthly Searches")

    XLSX.writeFile(wb, xlsx_fileaname)
